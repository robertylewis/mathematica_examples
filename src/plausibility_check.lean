...
/-!
`plausibility_check` tries to find an assignment to the free variables that appear in the context
...
meta def plausibility_check_aux (hs : list expr) (xs : list expr) : tactic unit :=
do t ← target, nt ← to_expr ```(¬ %%t),
   ...
   if n > 0 then
      fail "plausibilty check: the negation of the goal is consistent with hypotheses"
   else skip

/-- The main `plausibility_check` tactic. -/
meta def tactic.interactive.plausibility_check : tactic unit :=
do hyps ← local_context,
   ...
   plausibility_check_aux hypt vars
end
